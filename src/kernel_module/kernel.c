...
static int van_data_hook(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (idx >= FW_GEN_PRE_ALLOC) {
		printk(KERN_INFO
		       "NetVanguard: The radix tree database is full; this rule overwrites at zero index\n");
		WRITE_ONCE(idx, 0);
	}
	...
	if (db) {
		...
		printk(KERN_INFO "NetVanguard: # %ld SIDE: %s\n", idx,
		       (GET_SIDE(db->flags) == INPUT) ? "INPUT" : "OUTPUT");
		printk(KERN_INFO "NetVanguard: # %ld HOOK: %s\n", idx,
		       (GET_HOOK_TYPE(db->flags) == SOURCE) ? "SOURCE" :
							      "DESTINATION");
		printk(KERN_INFO "NetVanguard: # %ld RULE: %s\n", idx,
		       (GET_RULE_TYPE(db->flags) == ACCEPT) ? "ACCEPT" :
		       (GET_RULE_TYPE(db->flags) == BLOCK)  ? "BLOCK" :
							      "REJECT");
		...
		printk(KERN_ERR
		       "NetVanguard: The radix tree database idx[%ld] does not exist\n",
		       idx);
		return -ENOMEM;
	}
...
static const struct genl_ops van_ops[] = { { .doit = van_data_hook,
...
static struct genl_family van_family = { .name = FW_NETLINK_NAME,
...
static int pack_reply_message(struct genl_info **info, u32 status)
{
	...
	if (skb == NULL) {
		printk(KERN_ERR
		       "NetVanguard: Failed to allocate message for reply\n");
		return -ENOMEM;
	}
...
static unsigned int block_both(const struct sk_buff *skb,
			       const struct van_str_rule_t *rules)
{
	if (skb && rules) {
		...
		case SOURCE:
			if (iph->saddr == rules->ip && tcph->source == port)
				return NF_DROP;
			break;
		case DESTINATION:
			if (iph->daddr == rules->ip && tcph->dest == port)
				return NF_DROP;
			break;
		default:
			return NF_ACCEPT;
...
static unsigned int block_ip(const struct sk_buff *skb,
			     const struct van_str_rule_t *rules)
{
	if (skb && rules) {
		struct iphdr *iph = ip_hdr(skb);
		...
		switch (GET_HOOK_TYPE(rules->flags)) {
		case SOURCE:
			if (iph->saddr == rules->ip)
				return NF_DROP;
			break;
		case DESTINATION:
			if (iph->daddr == rules->ip)
				return NF_DROP;
			break;
		default:
			return NF_ACCEPT;
...
static unsigned int block_port(const struct sk_buff *skb,
			       const struct van_str_rule_t *rules)
{
	if (skb && rules) {
		u16 port = htons(rules->port);
...
static unsigned int ntf_post_hook(void *priv, struct sk_buff *skb,
...
static unsigned int ntf_pre_hook(void *priv, struct sk_buff *skb,
				 const struct nf_hook_state *state)
{
	struct van_str_rule_t *rules;
	struct genradix_iter iter;
	...
	genradix_for_each(&rule_genradix, iter, rules) {
		...
		if (rules->ip != 0 && rules->port != 0) {
			if (block_both(skb, rules) == NF_DROP) {
				pr_info_ratelimited(
					"NetVanguard: DROP BOTH PRE\n");
				return NF_DROP;
			}
		} else if (rules->ip != 0) {
			if (block_ip(skb, rules) == NF_DROP) {
				pr_info_ratelimited(
					"NetVanguard: DROP IP PRE\n");
				return NF_DROP;
			}
		} else if (rules->port != 0) {
			if (block_port(skb, rules) == NF_DROP) {
				pr_info_ratelimited(
					"NetVanguard: DROP PORT PRE\n");
				return NF_DROP;
			}
		}
	}
...
static int __init netvanguard_init(void)
{
	...
	if (ret < 0) {
		printk(KERN_ERR
		       "NetVanguard: Failed to register Netlink family error id[%d]\n",
		       ret);
		return ret;
	}
	...
	if (ret < 0) {
		...
		printk(KERN_ERR
		       "NetVanguard: Failed to register Netfilter pre-routing hook\n");
		return ret;
	}
	...
	if (ret < 0) {
		genl_unregister_family(&van_family);
		printk(KERN_ERR
		       "NetVanguard: Failed to register Netfilter post-routing hook\n");
		return ret;
	}
