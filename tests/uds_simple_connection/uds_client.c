...
int main() {
    ...
    ssize_t send_bytes = uds_socket_send(&uds_config, send_msg, 15, SERVER_ADDR, ADDR_LEN);
    if(send_bytes < 0) {
        ERROR("uds send error!");
        return -1;
    }
    SUCCESS("uds client send %ld bytes successfull [%s] message", send_bytes, send_msg);

    ssize_t recv_bytes = uds_socket_recv(&uds_config, send_msg, BUFFER_SIZE);
    if(recv_bytes < 0)
    {
        ERROR("uds recv failed!");
        return -1;
    }
    SUCCESS("uds client recv %ld bytes succesfull [%s] message", recv_bytes, send_msg);
