...
int main()
{
	...
	ssize_t recv_bytes =
		uds_socket_recv(&uds_config, recv_msg, BUFFER_SIZE);
	if (recv_bytes < 0) {
		ERROR("uds recv failed!");
		return -1;
	}
	SUCCESS("uds server recv %ld bytes succesfull [%s] message", recv_bytes,
		recv_msg);

	strncpy(recv_msg, "hello client", 13);
	ssize_t send_bytes = uds_socket_send(&uds_config, recv_msg, 13,
					     CLIENT_ADDR, ADDR_LEN);
	if (send_bytes < 0) {
		ERROR("uds send error!");
		return -1;
	}
	SUCCESS("uds server send %ld bytes successfull [%s] message",
		send_bytes, recv_msg);
